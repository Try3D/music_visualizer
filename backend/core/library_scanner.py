...
class MusicLibraryScanner:
        ...
        if self.cache_file.exists():
            try:
                with open(self.cache_file, 'r', encoding='utf-8') as f:
                    ...
                    self.tracks = [TrackMetadata.from_dict(track) for track in cache_data]
    ...
    def _calculate_file_hash(self, filepath: str) -> str:
        ...
        try:
            with open(filepath, "rb") as f:
                # Read first and last 64KB for speed on large files
                chunk = f.read(65536)
    ...
    def _extract_metadata_from_path(self, filepath: str) -> Tuple[str, str, str, Optional[int]]:
        """Extract artist, album, title from file path using the pattern"""
        path = Path(filepath)
        
        # Get relative path from the music root
        parts = path.parts
        
        # Expected structure: {artist}/{album}/{track_no} - {track_name}.{ext}
        if len(parts) >= 3:
            artist = parts[-3]  # Artist is the third-to-last part
            album = parts[-2]   # Album is the second-to-last part
            filename = parts[-1]
            
            # Extract title and track number from filename
            stem = path.stem
            
            # Pattern: {track_no} - {track_name} or {track_no}. {track_name}
    ...
    def scan_directory(self, music_dir: str, force_rescan: bool = False) -> List[TrackMetadata]:
        ...
        # Clear existing tracks if force rescan
        if force_rescan:
            self.tracks = []
        ...
        for audio_file in audio_files:
            ...
            # Skip if already cached and current (unless force rescan)
            if not force_rescan and self._is_file_cached_and_current(filepath):
                continue
            
            try:
                # Extract metadata from path structure
                artist, album, title, track_number = self._extract_metadata_from_path(filepath)
                
                # Extract metadata from audio tags
                tag_metadata = self._extract_metadata_from_tags(filepath)
                
                # Prefer tag metadata over path metadata where available
                ...
                # Parse track number from tags if available
                if 'track' in tag_metadata:
                    try:
                        track_str = tag_metadata['track'].split('/')[0]  # Handle "5/12" format
                ...
                if 'year' in tag_metadata:
                    try:
                        year_str = tag_metadata['year'][:4]  # Take first 4 digits
                        year = int(year_str)
                    except (ValueError, AttributeError):
                        pass
    ...
    def get_tracks_by_artist(self, artist: str) -> List[TrackMetadata]:
        ...
        return [track for track in self.tracks if track.artist.lower() == artist.lower()]
    ...
    def get_tracks_by_genre(self, genre: str) -> List[TrackMetadata]:
        ...
        return [track for track in self.tracks if track.genre and track.genre.lower() == genre.lower()]
...
if __name__ == "__main__":
    # Test the scanner on our sample tracks
    ...
    for key, value in stats.items():
        if key not in ['artists', 'genres']:  # Skip long lists for demo
            print(f"  {key}: {value}")
