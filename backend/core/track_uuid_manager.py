...
class TrackUUIDManager:
    ...
    def save_mapping(self):
        ...
        try:
            ...
            print(
                f"💾 Saved {len(self.uuid_to_track)} UUID mappings to {self.mapping_file}"
            )
        except Exception as e:
            print(f"❌ Error saving UUID mapping: {e}")

    def get_or_create_uuid(
        self, track_id: str, audio_file_path: Optional[str] = None
    ) -> str:
    ...
    def _parse_track_info(
        ...
        metadata = {"original_track_id": track_id, "audio_file_path": audio_file_path}
    ...
    def update_from_dna_database(self, dna_database):
        ...
        updated_count = 0
        for track_id, _ in dna_database.profiles.items():
    ...
    def rebuild_mapping(
        self, dna_database=None, audio_base_path: Optional[Path] = None
    ):
